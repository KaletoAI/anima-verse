...
when the difference is not walkable:
    ...
    dist < 1 m   -> a STEP:  |Δh| > game.max_step_height_m       (default 0.4)
    otherwise    -> a SLOPE: atan(|Δh| / dist) > game.max_slope_deg  (def. 40)

Two rules rather than one, because a 1 m wall and a 1 m rise over 20 m are not
the same obstacle. The predicate is pure (``relief.slope_blocks``) and the
CLIENT owns the identical one (``client3d/src/game/walk.ts`` ``slopeBlocks``,
hand-derived in ``client3d/scripts/smoke_walk_math.mjs``) — the two are checked
against the same table, section [1] below.
...
weight is 0).
  ...
  [1] THE PREDICATE, the same table the client checks (limits 0.4 m / 40°):
      ...
      0.3 m over 2 m   -> a SLOPE, atan(0.15) = 8.5308°     -> free
      0.3 m over 0.5 m -> a STEP, 0.3 ≤ 0.4                 -> free
      0.4 m over 0.5 m -> the limit itself passes (strict >) -> free
      −1.2 m over 0.5 m -> a drop is the same wall           -> blocked
      2 m over 2 m     -> atan(1) = 45° > 40                -> blocked
      1.6 m over 2 m   -> atan(0.8) = 38.6598° < 40         -> free
      1.6 m over 0.99 m -> the same rise is a STEP there    -> blocked
      0.5 m over 1 m   -> 1 m is ALREADY a slope (26.5651°) -> free
...
def place(name: str, x: float, z: float, *, yaw: float = 0.0,
          tile_rotation: int = 0, with_relief: bool = True) -> str:
    """An area-detail location of 8 m edge with ONE opening on its N edge."""
    ...
    if with_relief:
        fields["relief"] = {"amplitude_m": AMPLITUDE_M, "seed": SEED,
                            "wave_m": WAVE_M}
...
set_known_locations(AVATAR, [CLIFF, TURNED, SPUN, PLAIN])
