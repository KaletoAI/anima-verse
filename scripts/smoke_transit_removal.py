...
Hand-derived expectations, on a throwaway world with four records:
  * "Inn"    — ordinary location, placed at (10, 20), variant_seed 7
                                                              -> stays, seed stripped
  ...
  * "f-1"    — template_location_id="<Forest id>", (30, 40)   -> DELETED (E5)
  ...
  The Inn is the SURVIVOR half of the rule: it carries none of the two
  delete markers, so it stays, but its ``variant_seed`` is one of the three
  dead keys and is stripped — one field, on one record, hence
  fields_stripped 1. (Without such a record the "no legacy keys" check
  below would be vacuous: nothing would ever have had to be stripped.)
  migrate_transit_places_once() -> {"deleted_locations": 2,
  "deleted_galleries": 1, "fields_stripped": 1}; a second run -> all zeros
  (the deletions are gone and the Inn has no dead key left).
  After it, list_locations() has exactly the two survivors and neither
  carries "passable", "template_location_id" or "variant_seed".
...
for l in data["locations"]:
    if l["id"] == inn["id"]:
        l.update({"pos_x": 10.0, "pos_z": 20.0, "variant_seed": 7})
    if l["id"] == lake["id"]:
...
check("first run", r1, {"deleted_locations": 2, "deleted_galleries": 1, "fields_stripped": 1})
...
check("no legacy keys",
      [k for l in list_locations()
       for k in ("passable", "template_location_id", "variant_seed") if k in l],
      [])
