...
THE RULES, by hand — from the task brief:
  ...
  A) A new temporary NPC gets the standard skill set. The set is TEMPLATE
     ...
     `apply_npc` switches on every ID of that list THE SKILL MANAGER KNOWS
     and skips the rest silently — the list is written for a full
     installation, and packages that ship in a separate repo (or one somebody
     removed) must not break an NPC spawn. Hand-derived expectations:
       ...
       [2] After `apply_npc` every id of the list that the running skill
           ...
           and no id it does not know has a config file at all. Which half is
           which depends on the installation, so the expectation is the
           INTERSECTION with `get_skill_manager().skills`, computed here —
           plus the ids this repo itself ships, which must always be on.
       [3] Sleep is not planned for temporary NPCs: `sleep` and `wakeup` are
           installed in this repo, are NOT in the list, and must therefore
           stay off — the absence of a config file is the "off" state
           (`_get_agent_skills` treats a missing config as inactive for
           non-ALWAYS_LOAD skills).
...
KNOWN_SKILL_IDS = {getattr(s, "SKILL_ID", "") for s in get_skill_manager().skills}
...
check("nothing outside the list was written",
      sorted(p.stem for p in get_character_skills_dir(NPC).glob("*.json")
             if p.stem not in EXPECTED_SET), [])

# ── [3] sleep is not planned for temporary NPCs ─────────────────────────────
print("[3] the sleep verbs stay off")
check("sleep + wakeup ARE installed here", sorted(
    sid for sid in ("sleep", "wakeup") if sid in KNOWN_SKILL_IDS),
    ["sleep", "wakeup"])
check("but the NPC has no config for either",
...
try:
    ...
    _activated = npc_ops.activate_default_skills(NPC, "npc-temporary")
finally:
...
check("and the unknown one has no config",
      get_character_skill_config(NPC, "no_such_skill_at_all"), {})
