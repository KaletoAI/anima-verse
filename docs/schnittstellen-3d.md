...
| `scripts/smoke_model_surface.py` | das Oberflächen-Raster (v6): **part 0** die Gründe eines Backens, **part 1** die Zellenregel Knoten für Knoten an einer rein in Python geschriebenen Mini-GLB (80/20/20/90 cm + `null`) samt beider Boxen, `extent_snapped` unter zwei Fixen und der Gültigkeitsprüfung, dazu der WELT-Schritt (`target_m` 20 → `s` 10 → `step` 0,025 Modell-Einheiten, 81×81 Knoten, dieselben 80/20/90 cm an den verschobenen Indizes; `target_m` 0 lässt 9×9 @ 0,25; die `max_cells`-Kappe greift NACH der Umrechnung — mit 100 Zellen verdoppelt sich 0,025 bis auf 0,4 Modell-Einheiten = 4 m Welt (6×6); eine v1-Datei liest sich als `stale`), **part 2** die Sampler-Handtabelle |
