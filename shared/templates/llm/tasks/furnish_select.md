...
- Pick nothing when nothing fits the room's purpose, when the catalog is empty, or when the budget is 0. An empty list is a valid and often correct answer.
...
{% else %}
Footprint budget for new picks: 0 m² — the room is already as full as it may get.
{% endif %}
...
{% else %}
The furniture library holds no library items you may pick from. There is nothing to choose — answer with {"existing": []}.
{% endif %}
