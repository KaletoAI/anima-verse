        ...
        "sub_arrays": {
            ...
            "backends": {
                ...
                "fields": {
                    ...
                    "mesh_rig": {"type": "select", "label": "Rig", "choices": ["mixamo", "generic", "none"], "default": "mixamo", "triggers_rerender": True, "description": "What kind of skeleton this alias produces — it decides what may use it. 'mixamo' (e.g. Trellis2-Humanoid-*): 52-bone humanoid rig, delivered as ONE GLB with embedded textures; the shared animation clips apply. 'generic' (e.g. Trellis2-Generic-*): no standard skeleton, delivered as an FBX plus a separate basecolor PNG; NO animation clips. 'none' (e.g. Trellis2-Building-*): unrigged GLB with embedded texture for static props / building models — never offered for characters (only humanoid->mixamo and non-humanoid->generic are), only for location building models. Humanoid characters are matched to mixamo backends, non-humanoid ones (template feature 'humanoid' off) to generic backends.", "applicable_for": ["openai_mesh"]},
                    ...
                    "no_fingers": {"visible_when": {"mesh_rig": "mixamo"}, "half": True, "type": "bool", "label": "No fingers", "default": True, "description": "Alias param 'no fingers': skip separate finger geometry (more robust hands at this mesh resolution). Humanoid aliases only — the generic ones do not take this param.", "applicable_for": ["openai_mesh"]},
