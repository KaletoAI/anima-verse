...
SECTIONS = {
    ...
    "image_generation": {
        ...
        "fields": {
            ...
            "scene_prompt_multi_ref": {"type": "text", "label": "Scene Prompt (multi_ref)", "placeholder": "The exact {setting} from the first reference image, keeping its layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. The person reference images provide IDENTITY ONLY (face, hair, body, outfit) — IGNORE the pose and background they show; each person's pose follows the text. People: {people}", "legacy_defaults": list(SCENE_PROMPT_LEGACY_DEFAULTS), "description": "Prompt template for the multi_ref render mode. EMPTY = built-in default (shown greyed). Placeholders: {setting} = 'room'/'outdoor location'/'place' (from the location's indoor flag), {count} = e.g. 'exactly two people', {people} = person list with reference bindings and poses, {label} = room name (optional)."},
