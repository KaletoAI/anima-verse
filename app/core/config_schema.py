...
SECTIONS = {
    ...
    "image_generation": {
        ...
        "fields": {
            ...
            "animal_pose_prompt": {"type": "text", "label": "Mesh pose prompt (non-humanoid)", "placeholder": "standing still on all fours in a neutral rest pose, seen from a three-quarter side angle, the whole body from nose to tail visible, all four legs straight, clearly separated and not overlapping, head level and facing forward, tail extended away from the body", "description": "POSE LAYER ONLY for the mesh-input render of NON-humanoid characters (template feature 'humanoid' off) — a T-pose is meaningless on four legs. Framing comes from the 'tpose_animal' use-case style (nose-to-tail instead of arm span). EMPTY = built-in default."},
