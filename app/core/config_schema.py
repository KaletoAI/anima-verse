...
SECTIONS = {
    ...
    "image_generation": {
        ...
        "fields": {
            ...
            "scene_prompt_collage": {"type": "text", "label": "Scene Prompt (collage)", "default": "{label}: the people were pasted onto this room photo — seamlessly integrate them into the scene. Keep the room and keep every person exactly where and as they are: same position, same size, same pose, {count} and NO ONE else — no additional people, no duplicates. Blend lighting, shadows, color grading, reflections and edges so it looks like one natural photograph", "description": "Prompt template for the collage render mode. Placeholders: {label} = room name, {count} = e.g. 'exactly two people'. Tune per edit workflow (e.g. Krea) without code changes."},
