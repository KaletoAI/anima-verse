...
SECTIONS = {
    ...
    "image_generation": {
        ...
        "fields": {
            ...
            "tpose_prompt": {"type": "text", "label": "T-pose prompt (humanoid)", "placeholder": "T-pose, standing upright facing the camera, arms raised straight out to the sides at exact shoulder height, fully extended and parallel to the floor, body and arms forming the letter T, palms facing down toward the floor, fingers straight, extended and slightly spread apart, thumbs pointing forward, legs straight and slightly apart", "description": "POSE LAYER ONLY for the T-pose reference render of HUMANOID characters (input for image-to-3D/rigging chains) — placed at the START of the prompt so the pose outweighs the outfit text. Palms face DOWN: that is the Mixamo bind pose the rig is built against. Turned palms would show the mesher more hand detail, but the hand bones then bind 90° off and every animation clip twists the hands. Framing/lighting/background come from the 'tpose' use-case style. EMPTY = built-in default."},
