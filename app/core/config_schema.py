...
# The reference-render pose prompts are owned by model_refs — the schema shows
# them as greyed placeholders only (one source, no drifting copy).
from app.core.model_refs import (  # noqa: E402
    ANIMAL_POSE_PROMPT_DEFAULT,
    TPOSE_BACK_PROMPT_DEFAULT,
    TPOSE_LEFT_PROMPT_DEFAULT,
    TPOSE_PROMPT_DEFAULT,
    TPOSE_RIGHT_PROMPT_DEFAULT,
)
...
SECTIONS = {
    ...
    "chat": {
        ...
        "fields": {
            ...
            "chattiness": {"type": "float", "label": "Chattiness", "default": 0.5, "min": 0, "max": 1, "step": 0.05, "description": "How likely a bystander chimes in on a line that was not addressed to them (0 = never, 1 = always). At most one bystander answers per line; a line with a named addressee invites third parties far less than a line spoken to the room. A location can override this value for its own rooms (Chattiness field in the location editor)."},
            "pair_window_game_minutes": {"type": "int", "label": "Conversation pair window (game minutes)", "default": 5, "min": 1, "max": 120, "description": "Two characters who addressed each other within this window count as talking to each other; bystanders do not interrupt a pair. GAME time, so a frozen world freezes the window too."},
            "avatar_floor_timeout_minutes": {"type": "int", "label": "Avatar floor timeout (minutes)", "default": 8, "min": 0, "max": 120, "description": "While the avatar is in the room and has spoken within this real-time window, the NPCs get one reaction round per avatar line and then wait — the player keeps the floor. After the timeout the room falls back to the normal round limit. SYSTEM time (real minutes)."},
        },
    },
    "memory": {
        ...
        "fields": {
            ...
            "chat_retention_days": {"type": "int", "label": "Chat retention (days)", "default": 90, "min": 0, "max": 3650, "description": "Raw chat messages older than this are deleted once their game day has been summarized — a daily summary, or the week/season summary that day summary was later folded into. A game day that was never rolled up keeps its messages regardless of age, so nothing is lost that no summary preserved. 0 = keep forever. SYSTEM time (real days), like the consolidation thresholds above."},
    ...
    "image_generation": {
        ...
        "fields": {
            ...
            "tpose_prompt": {"type": "text", "label": "T-pose prompt (humanoid)", "placeholder": TPOSE_PROMPT_DEFAULT, "description": "POSE LAYER ONLY for the T-pose reference render of HUMANOID characters (input for image-to-3D/rigging chains) — placed at the START of the prompt so the pose outweighs the outfit text. Palms face FORWARD toward the camera: shows the mesher the most hand detail (note: the Mixamo bind pose has palms down — if hands come out twisted in animation, this is the knob to revisit). Legs are demanded CLEARLY APART with visible space between them — a clean separation, not a straddle: what the mesher needs is the gap, because touching legs fuse into one mesh column. The over-ask that used to sit here (a wide stance, feet wider than the shoulders) moved into the negative prompt, which pushes away touching legs/ankles and a narrow stance. Framing/lighting/background come from the 'tpose' use-case style. EMPTY = built-in default."},
            "animal_pose_prompt": {"type": "text", "label": "Mesh pose prompt (non-humanoid)", "placeholder": ANIMAL_POSE_PROMPT_DEFAULT, "description": "POSE LAYER ONLY for the mesh-input render of NON-humanoid characters (template feature 'humanoid' off) — a T-pose is meaningless on four legs. The symmetry comes from the POSE (head straight ahead in the body's direction, not at the camera; mouth closed), the 3/4 view only keeps body length and leg spacing visible. Framing/background come from the 'tpose_animal' use-case style (nose-to-tail instead of arm span). EMPTY = built-in default."},
            ...
            # more render per outfit combination.
            "tpose_back_prompt": {"type": "text", "label": "Back view prompt", "placeholder": TPOSE_BACK_PROMPT_DEFAULT, "description": "POSE LAYER ONLY for the extra BACK view of the T-pose reference render (enabled per character on the character editor's 3D tab). No face words here on purpose: every facial token pulls a face into the image and the model then turns the character toward the camera or paints a face onto the back of the head — the expression layer is left out entirely for this view. Framing, lighting and negatives come from the 'tpose_back' use-case style. EMPTY = built-in default."},
            "tpose_left_prompt": {"type": "text", "label": "Left profile prompt", "placeholder": TPOSE_LEFT_PROMPT_DEFAULT, "description": "POSE LAYER ONLY for the extra LEFT profile of the T-pose reference render (enabled per character on the character editor's 3D tab). The text describes what a T-pose profile actually shows — the near arm strongly foreshortened, pointing at the camera and hiding the far arm — because 'toward/away from the camera' alone made the render drop the arms forward and down. Framing, lighting and negatives come from the 'tpose_side' use-case style (shared with the right profile). EMPTY = built-in default."},
            "tpose_right_prompt": {"type": "text", "label": "Right profile prompt", "placeholder": TPOSE_RIGHT_PROMPT_DEFAULT, "description": "POSE LAYER ONLY for the extra RIGHT profile of the T-pose reference render (enabled per character on the character editor's 3D tab). Same text as the left profile with the side flipped. Framing, lighting and negatives come from the 'tpose_side' use-case style. EMPTY = built-in default."},
        ...
        "sub_arrays": {
            ...
            "backends": {
                ...
                "fields": {
                    ...
                    "remove_background": {"half": True, "type": "bool", "label": "Remove background", "default": True, "description": "Alias param 'input_remove_background': cut the subject out before meshing. Keep ON for renders on a plain background. Input images that already have no background (a real alpha cut-out) are sent with removal OFF automatically, whatever this says.", "applicable_for": ["openai_mesh"]},
    ...
    "ui": {
        ...
        "fields": {
            ...
            "downscale_enabled": {
                ...
                "default": True,
                "description": "Re-encode generated item images at a lower resolution after the backend returns them. Location backgrounds (day/night/scene) keep their full resolution; only the on-disk copy shrinks.",
            },
