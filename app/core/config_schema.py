        ...
        "sub_arrays": {
            ...
            "backends": {
                ...
                "fields": {
                    ...
                    "category": {"type": "select", "label": "Category", "choices": ["txt2img", "img2img", "inpaint", "img2mesh"], "default": "txt2img", "triggers_rerender": True, "description": "What this backend is for — set on EVERY type, so use-case routing can pick the right one. 'txt2img' = generate from text (reference images may still be sent when the model has slots). 'img2img' = edit/transform an input image. 'inpaint' = canvas + mask (openai_diffusion: POST /v1/images/edits); inpaint backends are kept OUT of normal image matching and are only offered as inpaint targets (map fit/edge dialog). 'img2mesh' = image to 3D mesh (openai_mesh)."},
