...
# template updates keep flowing. Append here whenever the default changes.
SCENE_PROMPT_LEGACY_DEFAULTS = (
    "{label}: the exact room from the first reference image, keeping the room layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. The person reference images provide IDENTITY ONLY (face, hair, body, outfit) — IGNORE the pose and background they show; each person's pose follows the text. People: {people}",
    "The exact room from the first reference image, keeping the room layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. The person reference images provide IDENTITY ONLY (face, hair, body, outfit) — IGNORE the pose and background they show; each person's pose follows the text. People: {people}",
    "{label}: the exact room from the reference image, keeping the room layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. People: {people}",
    "The exact room from the reference image, keeping the room layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. People: {people}",
    "The exact {setting} from the first reference image, keeping its layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. The person reference images provide IDENTITY ONLY (face, hair, body, outfit) — IGNORE the pose and background they show; each person's pose follows the text. People: {people}",
    "The exact {setting} from the first reference image, keeping its layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. Some people have a reference image: it provides their IDENTITY ONLY (face, hair, body, outfit) — IGNORE the pose and background it shows. People without a reference image are fully described in the text. Every person's pose follows the text. People: {people}",
    "The exact {setting} from the first reference image, keeping its layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. A person's reference image provides their IDENTITY ONLY (face, hair, body, outfit) — IGNORE the pose and background it shows; every person's pose follows the text. People: {people}",
    "The exact {setting} from the reference image, keeping its layout, lighting and perspective. Compose {count} into the scene and NO ONE else — each person appears exactly once, no additional people, no duplicates. People: {people}",
)
...
SECTIONS = {
    ...
    "chat": {
        ...
        "fields": {
            ...
            "anti_rep_max": {"type": "float", "label": "Temperature ceiling", "default": 0.9, "min": 0.7, "max": 2, "step": 0.05, "description": "Upper bound for the reactively-raised temperature. 0.90 is the highest value for which clean output was actually measured; 1.10 is measurably broken."},
            "anti_rep_lookback": {"type": "int", "label": "Repetition lookback (turns)", "default": 6, "min": 2, "max": 20, "description": "How many of the character's recent replies are fuzzy-checked for near-duplicates to drive the reactive bump. Note for thought turns: these are journal entries, so six of them can span several days — see the age limit below."},
            "anti_rep_max_age_hours": {"type": "int", "label": "Repetition age limit (game hours)", "default": 12, "min": 0, "max": 168, "description": "Journal entries older than this no longer count as a repetition on the thought path. Without it, a duplicate from two days ago penalises every turn today: measured on one evening, the duplicates driving a bump were 1.2 to 91.6 hours old (median 49). GAME time, so a frozen world freezes the window too. 0 = no age limit, count by entries only."},
        },
    },
    "memory": {
        ...
        "fields": {
            ...
            "max_semantic": {"type": "int", "label": "Max facts", "default": 50, "min": 10, "max": 200, "description": "Maximum number of semantic memories per character (hard cap, weakest removed at consolidation). Global default — override per NPC in the character editor (Memory section)."},
            "max_commitments": {"type": "int", "label": "Max commitments", "default": 20, "min": 1, "max": 200, "description": "Maximum number of commitment memories per character (count cap at consolidation; 'important'/importance≥4 protected — age rules apply separately). Global default — override per NPC in the character editor."},
            "max_episodic": {"type": "int", "label": "Max episodics", "default": 60, "min": 5, "max": 500, "description": "Maximum number of episodic memories awaiting the daily rollup (weakest removed at consolidation; 'important'/importance≥4 protected). Global default — override per NPC in the character editor."},
            "daily_entries_in_prompt": {"type": "int", "label": "Day entries in prompt", "default": 7, "min": 1, "max": 30, "description": "How many scene-based day recaps the chat prompt shows ('Earlier days'). Global default — override per NPC in the character editor (Memory section)."},
            "partner_days_in_prompt": {"type": "int", "label": "Partner days in prompt", "default": 5, "min": 1, "max": 30, "description": "How many per-partner chat day summaries the prompt shows. Global default — override per NPC."},
            "scenes_in_prompt": {"type": "int", "label": "Earlier scenes in prompt", "default": 8, "min": 1, "max": 30, "description": "How many not-yet-collapsed scenes the 'Earlier scenes today' block shows. Global default — override per NPC."},
            "scene_idle_minutes": {"type": "int", "label": "Scene idle (min)", "default": 30, "min": 1, "max": 1440, "description": "Silence threshold after which an open room conversation (scene) counts as ebbed away and is consolidated: LLM summary -> one episodic memory per participant, raw perceptions pruned. The agent loop checks about once a minute. SYSTEM time (real minutes)."},
            "scene_max_hours": {"type": "int", "label": "Scene max age (h)", "default": 6, "min": 0, "max": 168, "description": "Safety net against scenes that never fall idle (a room with constant traffic, or one waiting forever for an obligatory answer): an open scene whose FIRST line is older than this is consolidated regardless of silence or pending answers. The next utterance simply opens a new scene. 0 = off. SYSTEM time (real hours, like the idle threshold)."},
            "scene_max_perceptions": {"type": "int", "label": "Scene max perceptions", "default": 400, "min": 0, "max": 5000, "description": "Second safety net, by size: an open scene holding at least this many raw perceptions is consolidated regardless of silence or pending answers, so the perception context of a busy room cannot grow without bound. The next utterance simply opens a new scene. 0 = off."},
            "main_sleep_min_hours": {"type": "float", "label": "Main sleep threshold (h)", "default": 4, "min": 0, "max": 24, "step": 0.5, "description": "A sleep of at least this many GAME hours counts as main sleep and triggers the day consolidation on waking (scene block -> one day entry). GAME time (follows the game clock/factor)."},
            "max_block_open_hours": {"type": "int", "label": "Max open block (h)", "default": 30, "min": 1, "max": 168, "description": "Staleness fallback: an unconsolidated scene block older than this triggers the day consolidation even without a main sleep. SYSTEM time (real hours — a technical safety net, deliberately not game-coupled)."},
            "max_prompt_entries": {"type": "int", "label": "Max memories in prompt", "default": 20, "min": 1, "max": 100, "description": "Top-K most relevant memories shown in the chat prompt's memory section. Global default — override per NPC in the character editor."},
            ...
            "_grp_situational": {"type": "group_header", "label": "Situational memories (at the user turn)"},
            "situational_enabled": {"type": "bool", "label": "Situational memory block", "default": True, "description": "Attach the few memories that actually fit the incoming message to that message. Selection is embedding similarity (see Embedding section) between the message and the character's facts and open commitments — not importance, not age. The block hangs on the USER turn, never on the system prompt, so the cached prompt prefix stays byte-identical. Without an embedding backend the block is silently omitted."},
            "situational_max_entries": {"type": "int", "label": "Max situational entries", "default": 3, "min": 1, "max": 10, "description": "How many memories the block may carry at most. Deliberately small: this block sits on top of the memory section that is already in the system prompt, and it earns its place by being short and on-topic."},
            "situational_min_similarity": {"type": "float", "label": "Similarity threshold", "default": 0.75, "min": 0, "max": 1, "step": 0.01, "description": "Cosine similarity a memory must reach against the current message to be attached. Below it, nothing is attached at all — no block, no filler. 0.75 is deliberately strict: the default internal model (bge-small) puts even unrelated short texts around 0.6-0.7, so anything lower would attach noise on every message; a MiniLM-family model has a much lower floor, where 0.75 simply means 'clearly on topic'. Raise it if unrelated memories show up, lower it if the block never appears."},
        },
    },
    "image_generation": {
        ...
        "fields": {
            ...
            # built-in updates. Empty = built-in default from scene_render.py
            ...
            "blender_keep_original": {"type": "bool", "label": "Keep the raw mesh", "default": True, "description": "Store the untouched backend result in a 'raw/' folder beside the refined model. An img2mesh bake cannot be reproduced (GPU, not deterministic), so without the original a bad refinement is final. Costs disk; turn off only when space is tight."},
            "blender_auto_retexture": {"type": "bool", "label": "Re-encode textures automatically", "default": True, "description": "Re-encode a new model's textures to JPEG right after it is stored. Generated GLBs are 70–93 % texture, kept as uncompressed PNG — measured savings are 63–89 % of the whole file, with geometry, UVs and rig untouched. The result replaces the model only if it is smaller AND passes the same validation a fresh delivery faces; otherwise the original stays."},
            "blender_jpeg_quality": {"type": "int", "label": "Texture JPEG quality", "default": 85, "min": 40, "max": 100, "description": "Quality of the re-encoded texture maps. 85 is visually indistinguishable on generated meshes at roughly a fifth of the PNG size; below 70 block artefacts start showing on flat surfaces. Applies to the next re-encode, not to models already stored."},
            "blender_max_texture_size": {"type": "int", "label": "Max texture size (px)", "default": 0, "min": 0, "max": 4096, "description": "Downscale texture maps larger than this while re-encoding (0 = keep the resolution). Backends deliver 1024 or 2048 px; halving the edge quarters the pixel count and is the second-biggest saving after the JPEG step, but unlike it, this one is visible up close."},
            "blender_auto_lod": {"type": "bool", "label": "Build distance meshes on demand", "default": True, "description": "When a client asks for the reduced mesh of a model that has none, build it in the background — the full mesh is served meanwhile, and the next request gets the reduced one. Takes under a second per model and never blocks a response. This is a floor, not a ceiling: a better low-poly version made elsewhere can replace it at any time."},
            "blender_auto_normalize": {"type": "bool", "label": "Normalize new character meshes", "default": True, "description": "Scale a freshly generated or uploaded character mesh to the character's height and put its feet on the ground, once, on arrival. A Mixamo rig is measured at the crown joint, so hair and hats no longer shrink the body. The stored numbers become the real ones — clients stop rescaling by the bounding box. The original is kept whenever the result fails the same validation a fresh delivery faces."},
            "blender_auto_bake_vc": {"type": "bool", "label": "Bake vertex-colour meshes to textures", "default": True, "description": "A Gaussian-splat result (Triposplat) carries its colour in the vertices and has no UVs — nothing can re-encode, reduce or properly render it. On arrival such a mesh is UV-unwrapped, its holes closed, and the colours baked into a texture; afterwards it behaves like any other model. The original is kept under raw/."},
            "blender_bake_vc_target_tris": {"type": "int", "label": "Splat rebuild triangle budget", "default": 40000, "min": 0, "max": 500000, "description": "Before a vertex-colour mesh is unwrapped and baked, it is reduced to roughly this many triangles (0 = keep everything). Splat bakes arrive with hundreds of thousands of triangles; unwrapping those splits every seam and quadruples the file, while a reduced mesh keeps its look — the colours interpolate through the reduction and end up in the texture anyway. For comparison: a generated prop carries about 20k."},
            ...
            # shared/templates/pose/pose_catalog.json. EMPTY = built-in
            ...
            "tpose_prompt": {"type": "text", "label": "T-pose prompt (humanoid)", "placeholder": "T-pose, standing upright facing the camera, arms raised straight out to the sides at exact shoulder height, fully extended and parallel to the floor, body and arms forming the letter T, palms facing down toward the floor, fingers straight, extended and slightly spread apart, thumbs pointing forward, legs straight and slightly apart, hair tucked behind the shoulders, all upper-body garments worn closed, no open jacket and no loose fabric layers hanging in front of the body", "description": "POSE LAYER ONLY for the T-pose reference render of HUMANOID characters (input for image-to-3D/rigging chains) — placed at the START of the prompt so the pose outweighs the outfit text. Palms face DOWN: that is the Mixamo bind pose the rig is built against. Turned palms would show the mesher more hand detail, but the hand bones then bind 90° off and every animation clip twists the hands. Upper-body garments are demanded CLOSED: an open jacket or a layer hanging in front of the body floats next to the torso, and the image-to-3D bake fuses it with arms and chest instead of reconstructing it. Framing/lighting/background come from the 'tpose' use-case style. EMPTY = built-in default."},
            "animal_pose_prompt": {"type": "text", "label": "Mesh pose prompt (non-humanoid)", "placeholder": "standing still in a neutral pose on all four legs, three-quarter view, head facing straight forward in the direction of the body, not looking at the camera, all four legs straight and clearly apart, tail extended away from the body, mouth closed", "description": "POSE LAYER ONLY for the mesh-input render of NON-humanoid characters (template feature 'humanoid' off) — a T-pose is meaningless on four legs. The symmetry comes from the POSE (head straight ahead in the body's direction, not at the camera; mouth closed), the 3/4 view only keeps body length and leg spacing visible. Framing/background come from the 'tpose_animal' use-case style (nose-to-tail instead of arm span). EMPTY = built-in default."},
            "default_pose_prompt": {"type": "text", "label": "Default pose prompt", "placeholder": "standing with one hand on hip, weight shifted to one leg, shoulder slightly raised, chin up", "description": "POSE LAYER ONLY — replaces the built-in default pose everywhere it applies: expression variants without an activity, wardrobe outfit previews and the default-pose reference render. No style/framing/expression fragments here. EMPTY = built-in default from the '_default' entry of pose_catalog.json."},
            ...
            # OPTIONAL extra views of the T-pose reference render — input for
            # multi-view img2mesh backends. Off by default: every view is one
            # more render per outfit combination. Placeholders kept IN SYNC
            # with TPOSE_*_PROMPT_DEFAULT in app/core/model_refs.py.
            "tpose_view_back": {"half": True, "type": "bool", "label": "Extra mesh view: back", "default": False, "description": "Render a second T-pose reference from BEHIND after the front render and hand it to the mesh backend as an additional view. Without it the back of the head, the back of the outfit and the body depth are guessed by the mesher — this is the view that pays off most, and the one to switch on first. Humanoid characters only; aliases that declare only one image slot keep receiving just the front view."},
            "tpose_view_left": {"half": True, "type": "bool", "label": "Extra mesh view: left", "default": False, "description": "Render an additional strict LEFT side profile and hand it to the mesh backend. Profiles pin the body depth that a front view cannot show, but they are the shakiest of the three: in a profile one arm covers the other, and models regularly answer with a three-quarter view or drop the far arm. Check the render before trusting it. Humanoid characters only."},
            "tpose_view_right": {"half": True, "type": "bool", "label": "Extra mesh view: right", "default": False, "description": "Render an additional strict RIGHT side profile and hand it to the mesh backend. Same trade-off as the left profile — depth information for the mesher, but the least reliable view to render. Turning on both profiles doubles the extra cost and mostly buys symmetry. Humanoid characters only."},
            "tpose_back_prompt": {"type": "text", "label": "Back view prompt", "placeholder": 'back view, seen directly from behind, the back of the head and the heels toward the camera, face not visible, standing upright in T-pose, arms raised straight out to the sides at exact shoulder height, fully extended and parallel to the floor, palms facing down toward the floor, fingers straight and slightly spread apart, legs straight and slightly apart, hair tucked in front of the shoulders so the back stays visible, all garments worn closed', "description": "POSE LAYER ONLY for the extra BACK view of the T-pose reference render. No face words here on purpose: every facial token pulls a face into the image and the model then turns the character toward the camera or paints a face onto the back of the head — the expression layer is left out entirely for this view. Framing, lighting and negatives come from the 'tpose_back' use-case style. EMPTY = built-in default."},
            "tpose_left_prompt": {"type": "text", "label": "Left profile prompt", "placeholder": "strict left side profile view, seen exactly from the character's left side, facing to the left of the frame, standing upright in T-pose with both arms raised straight out to the sides at exact shoulder height, one arm extended toward the camera and one arm extended away from the camera, palms facing down toward the floor, fingers straight, legs straight and slightly apart, hair tucked behind the shoulders, all garments worn closed", "description": "POSE LAYER ONLY for the extra LEFT profile of the T-pose reference render. It spells out that one arm points at the camera and one away from it — otherwise the render folds both arms to the same side or loses the far one. Framing, lighting and negatives come from the 'tpose_side' use-case style (shared with the right profile). EMPTY = built-in default."},
            "tpose_right_prompt": {"type": "text", "label": "Right profile prompt", "placeholder": "strict right side profile view, seen exactly from the character's right side, facing to the right of the frame, standing upright in T-pose with both arms raised straight out to the sides at exact shoulder height, one arm extended toward the camera and one arm extended away from the camera, palms facing down toward the floor, fingers straight, legs straight and slightly apart, hair tucked behind the shoulders, all garments worn closed", "description": "POSE LAYER ONLY for the extra RIGHT profile of the T-pose reference render. Same text as the left profile with the side flipped. Framing, lighting and negatives come from the 'tpose_side' use-case style. EMPTY = built-in default."},
        ...
        "sub_arrays": {
            ...
            "backends": {
                ...
                "fields": {
                    ...
                    "category": {"type": "select", "label": "Category", "choices": ["txt2img", "img2img", "inpaint", "img2mesh", "mesh2mesh"], "default": "txt2img", "triggers_rerender": True, "description": "What this backend is for — set on EVERY type, so use-case routing can pick the right one. 'txt2img' = generate from text (reference images may still be sent when the model has slots). 'img2img' = edit/transform an input image. 'inpaint' = canvas + mask (openai_diffusion: POST /v1/images/edits); inpaint backends are kept OUT of normal image matching and are only offered as inpaint targets (map fit/edge dialog). 'img2mesh' = image to 3D mesh (openai_mesh). 'mesh2mesh' = an EXISTING mesh is reduced to a coarser one (openai_mesh, alias 'mesh-shrink'); such a backend is never offered for a normal 3D generation, only behind \"Create low variant\" in the model galleries."},
                    ...
                    "supports_negative_prompt": {"type": "select", "label": "Negative prompt", "choices": ["auto", "yes", "no"], "default": "auto", "visible_when": {"category": ["txt2img", "img2img"]}, "not_applicable_for": ["openai_mesh", "localai_video", "together_video", "openai_video"], "description": "Does this backend have a negative input? auto = decided by the prompt family (Image Family, or the model name when it is empty): natural-family models (Flux, Qwen-Image) are distilled / guidance-free and have NO negative input, keyword-family / CFG models (SD/SDXL, A1111, Z-Image with CFG > 1) have one. yes/no override that — Z-Image Turbo is the case auto gets wrong (keyword family, but guidance-free), so set it to no. Without a negative input every negative (use-case default + negations taken out of the subject + dialog input) is folded into the positive prompt as negations (\"no fog, no trees\") instead of being dropped."},
                    ...
                    # (GET /v1/generations/<alias>/schema) — all public names
                    # are input_*: images[input_image] + params{input_name,
                    # input_remove_background, input_face_num |
                    # input_num_gaussians, input_texture_resolution,
                    # input_no_fingers}. No prompt, no size, no families.
                    ...
                    "_mesh2mesh_note": {"visible_when": {"category": "mesh2mesh"}, "type": "note", "applicable_for": ["openai_mesh"], "text": (
                        "Mesh reduction (alias 'mesh-shrink'): the SOURCE MESH travels in the request "
                        "(files.input_mesh_path, max 64 MB) — no input image, no rig. Set Rig to 'none': "
                    ...
                    "no_fingers": {"visible_when": {"mesh_rig": "mixamo"}, "half": True, "type": "bool", "label": "No fingers", "default": True, "description": "Alias param 'input_no_fingers': rig the hands without separate fingers (more robust at this mesh resolution). Every family accepts the param, but it only takes effect in the humanoid rig stage.", "applicable_for": ["openai_mesh"]},
                    "face_num": {"half": True, "type": "int", "label": "Face count", "default": 0, "min": 0, "max": 500000, "description": "Alias param 'input_face_num': target polygon count of the generated mesh. Higher = more detail, bigger file, slower. 0 = send nothing and let the alias default stand — the right setting for the splat pipeline (Triposplat), which has no face param and uses 'input_num_gaussians' (default 10000) instead.", "applicable_for": ["openai_mesh"]},
                    "face_num_max": {"half": True, "type": "int", "label": "Face count limit", "default": 0, "min": 0, "max": 500000, "description": "Hard ceiling for the face count of THIS alias — a higher value (from the config or from the generate dialog) is clamped down to it. 0 = no limit. Needed where a backend does not fail on too many faces but hangs: Hunyuan3D freezes above 40000 and the job runs into the timeout.", "applicable_for": ["openai_mesh"]},
                    ...
                    "max_queue_wait": {"half": True, "type": "int", "label": "Max Queue Wait (s)", "default": 1200, "min": 30, "description": "Maximum time an async gateway job may sit QUEUED (GPU busy elsewhere) before it is abandoned as busy — separate from Max Wait, which budgets the RUNNING time only. Deliberately generous. Empty = 4× Max Wait. SYSTEM time.", "applicable_for": ["openai_video", "openai_mesh"]},
