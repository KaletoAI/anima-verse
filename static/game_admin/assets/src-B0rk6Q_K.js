import{i as e}from"./react-BRNZa73l.js";var t=315,n=45,r=.35,i=1,a=1,o=Math.PI/180;function s(e,t){return typeof e==`number`&&Number.isFinite(e)?e:t}function c(e,t,n){let r=e[t]?.[n];return typeof r==`number`&&Number.isFinite(r)?r:0}function l(e,l){let u=e?.heights;if(!u||u.length<2)return null;let d=u.length,f=u[0]?.length||0;if(f<2)return null;let p=e?.step_m??0;if(!(p>0))return null;let m=s(l?.azimuthDeg,t)*o,h=Math.min(90,Math.max(a,s(l?.altitudeDeg,n)))*o,g=Math.min(1,Math.max(0,s(l?.maxAlpha,r))),_=Math.max(0,s(l?.zFactor,i)),v=Math.cos(h),y=v*Math.sin(m),b=Math.sin(h),x=-v*Math.cos(m),S=b,C=new Uint8ClampedArray(f*d*4);for(let e=0;e<d;e+=1){let t=e>0?e-1:e,n=e<d-1?e+1:e,r=(n-t)*p;for(let i=0;i<f;i+=1){let a=i>0?i-1:i,o=i<f-1?i+1:i,s=(o-a)*p,l=_*(c(u,e,o)-c(u,e,a))/s,d=_*(c(u,n,i)-c(u,t,i))/r,m=Math.sqrt(l*l+d*d),h=Math.sqrt(m*m+1),v=(-l*y+b-d*x)/h,w=Math.min(1,.5*(v>0?v:0)/S),T=Math.round(255*w),E=Math.round(255*g*(m/h)),D=(e*f+i)*4;C[D]=T,C[D+1]=T,C[D+2]=T,C[D+3]=E}}return{cols:f,rows:d,data:C}}function u(e,t,n,r=0){if(t!==`aligned`)return e*Math.PI*2;let i=Number.isFinite(Number(n))?Number(n)*Math.PI/180:0,a=Number.isFinite(r)?r:0,o=Math.PI*2;return((a+i)%o+o)%o}var d=2e3;function f(e,t,n){return`terrain:scatter:${e}:${t}${p(n)}`}function p(e){return Number.isFinite(e)?`:e${e}`:``}function m(e,t){let n=Number(t);if(!Number.isFinite(n)||n<=0)return;let r=Number(e);if(Number.isFinite(r))return Math.floor(r/(n*60))}function h(e){let t=2166136261;for(let n=0;n<e.length;n+=1)t^=e.charCodeAt(n),t=Math.imul(t,16777619);return t>>>0}function g(e,t,n){let r=Math.floor(Number(n));if(!Number.isFinite(r)||r<=1)return 0;let i=Math.floor(Number(t));return Number.isFinite(i)?((h(e)+i)%r+r)%r:0}function _(e,t){if(typeof e!=`number`||!Number.isFinite(e))return-1;let n=Math.floor(Number(t));return!Number.isFinite(n)||n<1?-1:Math.min(Math.max(Math.floor(e),0),n-1)}function v(e){let t=h(e)|0;return()=>(t=Math.imul(t^t>>>15,2246822519),t=Math.imul(t^t>>>13,3266489917),((t^=t>>>16)>>>0)/4294967296)}function y(e,t,n){let r=!1,i=n?.length??0;for(let a=0,o=i-1;a<i;o=a,a+=1){let[i,s]=n[a],[c,l]=n[o];s>t!=l>t&&e<(c-i)*(t-s)/(l-s)+i&&(r=!r)}return r}function b(e,t,n,r,i){let a=Math.cos(n),o=Math.sin(n),s=r-e,c=i-t;return{x:s*a-c*o,z:s*o+c*a}}function x(e,t,n){let r=e?.points,i=r?.length??0;if(i<3||!Number.isFinite(t)||!Number.isFinite(n))return!1;let a=!1;for(let e=0,o=i-1;e<i;o=e,e+=1){let i=r[e],s=r[o];if(!i||!s||i.length<2||s.length<2)return!1;let c=i[0],l=i[1],u=s[0],d=s[1];if(!Number.isFinite(c)||!Number.isFinite(l)||!Number.isFinite(u)||!Number.isFinite(d))return!1;l>n!=d>n&&t<(u-c)*(n-l)/(d-l)+c&&(a=!a)}return a}function S(e,t,n){let r=e?.points,i=r?.length??0;if(i<3||!Number.isFinite(t)||!Number.isFinite(n))return 1/0;let a=!1,o=1/0;for(let e=0,s=i-1;e<i;s=e,e+=1){let i=r[e],c=r[s];if(!i||!c||i.length<2||c.length<2)return 1/0;let l=i[0],u=i[1],d=c[0],f=c[1];if(!Number.isFinite(l)||!Number.isFinite(u)||!Number.isFinite(d)||!Number.isFinite(f))return 1/0;u>n!=f>n&&t<(d-l)*(n-u)/(f-u)+l&&(a=!a);let p=l-d,m=u-f,h=p*p+m*m,g=h<1e-18?0:((t-d)*p+(n-f)*m)/h;g=g<0?0:g>1?1:g;let _=t-(d+g*p),v=n-(f+g*m),y=Math.sqrt(_*_+v*v);y<o&&(o=y)}return a?0:o}function C(e,t,n,r){let i=Number(r);return i>0?S(e,t,n)<i:x(e,t,n)}var w=.5;function T(e,t){let n=Number(t);if(Number.isFinite(n)&&n>0)return n/2;let r=Number(e);return!Number.isFinite(r)||r<=0?0:r*w}function E(e){let t=Number(e?.x),n=Number(e?.z),r=Number(e?.half_w),i=Number(e?.half_d),a=Number(e?.yaw_deg);if(!Number.isFinite(t)||!Number.isFinite(n)||!(r>0)||!(i>0))return null;let o=(Number.isFinite(a)?a:0)*Math.PI/180,s=Math.cos(o),c=Math.sin(o),l=(e,r)=>[t+e*s+r*c,n-e*c+r*s];return{points:[l(-r,-i),l(r,-i),l(r,i),l(-r,i)]}}function D(e){let t=[];for(let n of e??[]){let e=E(n);e&&t.push(e)}return t}function O(e,t){let n=Number(e??t);return Number.isFinite(n)&&n>0?n:0}function ee(e,t,n){let r=Number(t),i=Number(e);if(!Number.isFinite(r)||r<=0||!Number.isFinite(i)||i<=0)return 0;let a=n??2e3,o=Math.min(Math.round(i/100*r),a);return o>=1?o:0}function k(e){let t=e.ring??[],n=Number(e.densityPer100m2),r=Number(e.areaM2);if(t.length<3||!Number.isFinite(n)||n<=0||!Number.isFinite(r)||r<=0)return[];let i=ee(r,n,e.maxPoints);if(i<1)return[];let a=1/0,o=1/0,s=-1/0,c=-1/0;for(let[e,n]of t)e<a&&(a=e),e>s&&(s=e),n<o&&(o=n),n>c&&(c=n);if(!Number.isFinite(a)||!Number.isFinite(o))return[];let l=e.rng??v(e.seed),d=e.footprints??[],f=e.clearM,p=e.occluders??[],m=Number(e.minSpacingM),h=Number.isFinite(m)&&m>0,b=m*m,x=h?new Map:null,S=Math.floor(Number(e.variantCount)),w=Number.isFinite(S)&&S>1,T=w?_(e.variant,S):-1,E=e.yawMode===`aligned`?e.axisAt:void 0,D=e.occupied,k=O(e.occupyR,e.clearM),A=e.occupyTag??e.seed,j=[],M=i*(e.triesPerPoint??12),N=0;for(;j.length<i&&M>0;){--M;let n=N;N+=1;let r=a+l()*(s-a),i=o+l()*(c-o),h=l();if(!y(r,i,t))continue;let _=!1;for(let e of p)if((e?.length??0)>=3&&y(r,i,e)){_=!0;break}if(_)continue;let v=!1;for(let e of d)if(C(e,r,i,f)){v=!0;break}if(v)continue;if(x){let e=Math.floor(r/m),t=Math.floor(i/m),n=!1;for(let a=-1;a<=1&&!n;a+=1)for(let o=-1;o<=1&&!n;o+=1){let s=x.get(`${e+o},${t+a}`);if(s)for(let[e,t]of s){let a=r-e,o=i-t;if(a*a+o*o<b){n=!0;break}}}if(n)continue}if(D&&D.blocks(r,i,k,A))continue;if(x){let e=`${Math.floor(r/m)},${Math.floor(i/m)}`,t=x.get(e);t?t.push([r,i]):x.set(e,[[r,i]])}D&&D.add(r,i,k,A);let O=u(h,e.yawMode,e.yawDeg,E?E(r,i):0);j.push(w?{x:r,z:i,yaw:O,variant:T>=0?T:g(e.seed,n,S)}:{x:r,z:i,yaw:O})}return j}var A=4e3,j=4096;function M(e){return Math.floor(e/64)}function N(e,t,n,r,i){return`terrain:scatter:${e}:${t}:${n},${r}${p(i)}`}function te(e,t){let n=e*64,r=t*64,i=n+64,a=r+64;return[[n,r],[i,r],[i,a],[n,a]]}function P(e,t,n,r){if(![e,t,n,r].every(e=>Number.isFinite(e))||n<e||r<t)return 0;let i=M(n)-M(e)+1,a=M(r)-M(t)+1;return!(i>0)||!(a>0)?0:i*a}function ne(e,t,n,r,i=j){let a=P(e,t,n,r);if(a<=0||a>i)return[];let o=M(e),s=M(n),c=M(t),l=M(r),u=[];for(let e=c;e<=l;e+=1)for(let t=o;t<=s;t+=1)u.push([t,e]);return u}function re(e){let t=e.ring??[];if(t.length<3)return[];let n=e.occupied;return k({ring:te(e.cx,e.cz),areaM2:4096,densityPer100m2:e.densityPer100m2,seed:e.seed,footprints:e.footprints,clearM:e.clearM,occluders:e.occluders,minSpacingM:e.minSpacingM,maxPoints:e.maxPoints??4e3,triesPerPoint:1,variantCount:e.variantCount,variant:e.variant,rng:e.rng,yawMode:e.yawMode,yawDeg:e.yawDeg,axisAt:e.axisAt,occupied:n?{blocks:(e,t,r,i)=>n.blocks(e,t,r,i),add:(e,r,i,a)=>{y(e,r,t)&&n.add(e,r,i,a)}}:void 0,occupyR:e.occupyR,occupyTag:e.occupyTag}).filter(e=>y(e.x,e.z,t))}var F=Math.PI*2,I=1e-4,L=1e-9,ie=1e5;function R(e){return(e%F+F)%F}function z(e,t,n,r,i,a){let o=i-n,s=a-r,c=o*o+s*s,l=c<1e-18?0:((e-n)*o+(t-r)*s)/c;l=l<0?0:l>1?1:l;let u=e-(n+l*o),d=t-(r+l*s);return Math.sqrt(u*u+d*d)}function B(e,t,n,r,i){let a=r-t,o=i-n,s=Math.hypot(a,o);if(!(s>0))return 0;let c=Math.atan2(a,o),l=(t+r)/2,u=(n+i)/2,d=o/s,f=-a/s;return R(y(l+d*I,u+f*I,e)?c:c+Math.PI)}function ae(e,t){let n=e?.length??0;if(n<3)return[];let r=Array.from({length:n},(e,t)=>t);if(t!==`longest`&&t!==`opposite`)return r;let i=r.map(t=>{let[r,i]=e[t],[a,o]=e[(t+1)%n],s=Math.hypot(a-r,o-i);return Number.isFinite(s)?s:0}),a=0;for(let e=1;e<n;e+=1)i[e]>i[a]&&(a=e);if(t===`longest`)return[a];let o=-1;for(let e=0;e<n;e+=1)e!==a&&(o<0||i[e]>i[o])&&(o=e);return a<o?[a,o]:[o,a]}function oe(e,t,n,r){let i=e?.length??0;if(i<3)return 0;let a=1/0,o=-1;for(let s=0,c=i-1;s<i;c=s,s+=1){if(r&&!r.includes(c))continue;let i=z(t,n,e[c][0],e[c][1],e[s][0],e[s][1]);i<a&&(a=i,o=s)}if(o<0)return 0;let s=o===0?i-1:o-1;return B(e,e[s][0],e[s][1],e[o][0],e[o][1])}function se(e,t,n){let r=e?.length??0;if(r<2)return 0;let i=1/0,a=0;for(let o=1;o<r;o+=1){let[r,s]=e[o-1],[c,l]=e[o],u=c-r,d=l-s;if(u*u+d*d<1e-18)continue;let f=z(t,n,r,s,c,l);f<i&&(i=f,a=R(Math.atan2(u,d)))}return a}function ce(e,t,n){if(e)return(t,n)=>se(e,t,n);let r=n===`longest`||n===`opposite`?ae(t,n):void 0;return(e,n)=>oe(t,e,n,r)}function le(e,t,n){let r=e.length,i=1/0;for(let a=0,o=r-1;a<r;o=a,a+=1){let r=z(t,n,e[o][0],e[o][1],e[a][0],e[a][1]);r<i&&(i=r)}return y(t,n,e)?i:-i}function V(e,t,n,r){let i=le(e,t,n);return{x:t,z:n,h:r,d:i,max:i+r*Math.SQRT2}}function H(e,t){e.push(t);let n=e.length-1;for(;n>0;){let t=n-1>>1;if(e[t].max>=e[n].max)break;let r=e[t];e[t]=e[n],e[n]=r,n=t}}function ue(e){let t=e[0],n=e.pop();if(e.length===0||n===void 0)return t;e[0]=n;let r=0;for(;;){let t=r*2+1,n=t+1,i=r;if(t<e.length&&e[t].max>e[i].max&&(i=t),n<e.length&&e[n].max>e[i].max&&(i=n),i===r)break;let a=e[i];e[i]=e[r],e[r]=a,r=i}return t}function de(e,t=.5){let n=e?.length??0;if(n<3)return{x:0,z:0,d:0};let r=1/0,i=1/0,a=-1/0,o=-1/0;for(let[t,n]of e){if(!Number.isFinite(t)||!Number.isFinite(n))return{x:0,z:0,d:0};t<r&&(r=t),t>a&&(a=t),n<i&&(i=n),n>o&&(o=n)}let s=a-r,c=o-i,l=Math.min(s,c);if(!(l>0))return{x:r+s/2,z:i+c/2,d:0};let u=Number.isFinite(t)&&t>0?t:.5,d=l/2,f=0,p=0,m=0;for(let t=0,r=n-1;t<n;r=t,t+=1){let[n,i]=e[t],[a,o]=e[r],s=n*o-a*i;f+=s,p+=(n+a)*s,m+=(i+o)*s}let h=Math.abs(f)>1e-12?V(e,p/(3*f),m/(3*f),0):V(e,r+s/2,i+c/2,0),g=V(e,r+s/2,i+c/2,0);g.d>h.d&&(h=g);let _=[];for(let t=r;t<a;t+=l)for(let n=i;n<o;n+=l)H(_,V(e,t+d,n+d,d));let v=_.length;for(;_.length>0&&v<ie;){let t=ue(_);if(!t)break;if(t.d>h.d&&(h=t),t.max-h.d<=u)continue;let n=t.h/2;H(_,V(e,t.x-n,t.z-n,n)),H(_,V(e,t.x+n,t.z-n,n)),H(_,V(e,t.x-n,t.z+n,n)),H(_,V(e,t.x+n,t.z+n,n)),v+=4}return{x:h.x,z:h.z,d:h.d}}function fe(e){let t=[],n=0;for(let r of e??[]){if(!r||r.length<2)return[];let[e,i]=r;if(!Number.isFinite(e)||!Number.isFinite(i))return[];let a=t[t.length-1];a&&Math.abs(a.x-e)<L&&Math.abs(a.z-i)<L?a.index=n:t.push({x:e,z:i,index:n}),n+=1}let r=t[0],i=t[t.length-1];if(t.length>1&&Math.abs(r.x-i.x)<L&&Math.abs(r.z-i.z)<L&&t.pop(),t.length<3)return[];let a=[],o=0;for(let e=0;e<t.length;e+=1){let{x:n,z:r,index:i}=t[e],{x:s,z:c}=t[(e+1)%t.length],l=Math.hypot(s-n,c-r);l>L&&(a.push({ax:n,az:r,bx:s,bz:c,len:l,start:o,end:o+l,index:i}),o+=l)}return a}function pe(e,t){let n=Number(t?.spacingM);if(!Number.isFinite(n)||n<=0)return[];let r=t?.offsetM===void 0?0:Number(t.offsetM);if(!Number.isFinite(r)||r<0)return[];let i=fe(e);if(i.length<3)return[];let a=typeof t.startM==`number`&&Number.isFinite(t.startM)&&t.startM>=0?Math.min(t.startM,n):n/2,o=t.maxPoints??2e3,s=Number(t.jitterM),c=s>0?t.jitterRng??v(`${t.seed??``}:jitter`):null,l=new Map,u=[],d=(t,i)=>{let d=0,f=0;for(let p=0;;p+=1){if(c){let e=(2*c()-1)*s;f=p===0?a+e:f+n+e,f<0&&(f=0)}else f=a+p*n;if(!(f<i-L)||u.length>=o)break;for(;d<t.length-1&&f>=t[d].end;)d+=1;let m=t[d],h=(f-m.start)/m.len;h=h<0?0:h>1?1:h;let g=m.ax+h*(m.bx-m.ax),_=m.az+h*(m.bz-m.az),v=l.get(m.index);v===void 0&&(v=B(e,m.ax,m.az,m.bx,m.bz),l.set(m.index,v));let y=v+Math.PI/2;u.push({x:g+Math.sin(y)*r,z:_+Math.cos(y)*r,axis:v,ordinal:u.length})}},f=t.sides;if(f!==`longest`&&f!==`opposite`)return d(i,i[i.length-1].end),u;let p=ae(e,f);for(let e of p){let t=i.find(t=>t.index===e);if(t&&(d([{...t,start:0,end:t.len}],t.len),u.length>=o))break}return u}function U(e,t,n){for(let r of e)if((r?.length??0)>=3&&y(t,n,r))return!0;return!1}function W(e,t,n,r){for(let i of e)if(C(i,t,n,r))return!0;return!1}function me(e,t){let n=pe(e,t);if(n.length===0)return[];let r=t.rng??v(t.seed),i=t.footprints??[],a=t.occluders??[],o=t.occupied,s=O(t.occupyR,t.clearM),c=t.occupyTag??t.seed,l=Math.floor(Number(t.variantCount)),d=Number.isFinite(l)&&l>1,f=d?_(t.variant,l):-1,p=[];for(let m of n){let n=r(),{x:h,z:_}=m,v=m.axis+Math.PI/2;if(!y(h+Math.sin(v)*I,_+Math.cos(v)*I,e)||U(a,h,_)||W(i,h,_,t.clearM)||o&&o.blocks(h,_,s,c))continue;o&&o.add(h,_,s,c);let b=u(n,t.yawMode,t.yawDeg,m.axis);p.push(d?{x:h,z:_,yaw:b,variant:f>=0?f:g(t.seed,m.ordinal,l)}:{x:h,z:_,yaw:b})}return p}function he(e,t){if((e?.length??0)<3)return[];let n=de(e);if(!(n.d>0))return[];let r=(t.rng??v(t.seed))(),{x:i,z:a}=n;if(U(t.occluders??[],i,a)||W(t.footprints??[],i,a,t.clearM))return[];let o=O(t.occupyR,t.clearM),s=t.occupyTag??t.seed;if(t.occupied&&t.occupied.blocks(i,a,o,s))return[];t.occupied&&t.occupied.add(i,a,o,s);let c=t.yawMode===`aligned`?t.axisAt?t.axisAt(i,a):oe(e,i,a):0,l=u(r,t.yawMode,t.yawDeg,c),d=Math.floor(Number(t.variantCount));if(!Number.isFinite(d)||d<=1)return[{x:i,z:a,yaw:l}];let f=_(t.variant,d);return[{x:i,z:a,yaw:l,variant:f>=0?f:g(t.seed,0,d)}]}var ge=.5,G={value:0},_e={value:{r:.62,g:.78,b:.91}};function ve(e){G.value=(G.value+(e||0))%3600}var K=null,ye=null;function be(e){let t=document.createElement(`canvas`);t.width=t.height=1;let n=t.getContext(`2d`);n.fillStyle=`#fff`,n.fillRect(0,0,1,1);let r=new e.CanvasTexture(t);return r.needsUpdate=!0,r}function xe(e){let t=document.createElement(`canvas`);t.width=t.height=256;let n=t.getContext(`2d`),r=n.createImageData(256,256),i=[[1,2,1,0],[2,-1,.7,1.7],[3,2,.4,3.1],[-2,3,.3,5]],a=(e,t)=>{let n=0;for(let[r,a,o,s]of i)n+=o*Math.sin(2*Math.PI*(r*e+a*t)+s);return n},o=1/256;for(let e=0;e<256;e++)for(let t=0;t<256;t++){let n=t/256,i=e/256,s=(a(n+o,i)-a(n-o,i))/(2*o),c=(a(n,i+o)-a(n,i-o))/(2*o),l=.02,u=-s*l,d=-c*l,f=Math.hypot(u,d,1)||1;u/=f,d/=f;let p=(e*256+t)*4;r.data[p]=Math.round((u*.5+.5)*255),r.data[p+1]=Math.round((d*.5+.5)*255),r.data[p+2]=Math.round((1/f*.5+.5)*255),r.data[p+3]=255}n.putImageData(r,0,0);let s=new e.CanvasTexture(t);return s.wrapS=s.wrapT=e.RepeatWrapping,s.anisotropy=4,s.needsUpdate=!0,s}var q=`#include <begin_vertex>`,J=`#include <normal_fragment_maps>`,Y=`#include <map_fragment>`,X=`#include <roughnessmap_fragment>`,Z=`#include <opaque_fragment>`,Se=!1;function Q(e){Se||(Se=!0,console.warn(`[scene-render] water shader: anchor "${e}" not found in this three version — the surface renders matte instead. One line to re-point.`))}function Ce(e,t){return t==null?new e.Color(16777215):typeof t==`number`||typeof t==`string`?new e.Color(t):new e.Color(t.r,t.g,t.b)}function we(e){let t=parseInt((e||`#3f7fb8`).slice(1),16);return{r:(t>>16&255)/255,g:(t>>8&255)/255,b:(t&255)/255}}function Te(e,t,n){let r={value:Math.max(t.wave_m??1.6,.05)},i={value:t.speed??.05},a={value:t.flow_speed??.5},o={value:t.sky_mix??.55},s={value:we(t.tint)},c={value:t.map_strength??.75},l={value:n};e.onBeforeCompile=e=>{if(e.uniforms.uTime=G,e.uniforms.uSky=_e,e.uniforms.uWaveM=r,e.uniforms.uSpeed=i,e.uniforms.uFlowSpeed=a,e.uniforms.uSkyMix=o,e.uniforms.uTint=s,e.uniforms.uMapStrength=c,e.uniforms.uMask=l,e.vertexShader.includes(q))e.vertexShader=`attribute vec2 aWaterFlow;
varying vec2 vWaterWorld;
varying vec2 vWaterUv;
varying vec2 vWaterFlow;
`+e.vertexShader.replace(q,`${q}\n  vWaterWorld = ( modelMatrix * vec4( transformed, 1.0 ) ).xz;
  vWaterUv = uv;
  vWaterFlow = aWaterFlow;`);else{Q(q);return}e.fragmentShader=`varying vec2 vWaterWorld;
varying vec2 vWaterUv;
varying vec2 vWaterFlow;
uniform float uTime;
uniform vec3 uSky;
...
`+e.fragmentShader,e.fragmentShader.includes(J)?e.fragmentShader=e.fragmentShader.replace(J,`
  ...
  #endif`):Q(J),e.fragmentShader.includes(Y)?e.fragmentShader=e.fragmentShader.replace(Y,`${Y}\n  diffuseColor.rgb = mix( uTint, diffuseColor.rgb, mix( 1.0, uMapStrength, texture2D( uMask, vWaterUv ).r ) );`):Q(Y),e.fragmentShader.includes(X)?e.fragmentShader=e.fragmentShader.replace(X,`${X}\n  roughnessFactor = mix( 0.85, roughnessFactor, texture2D( uMask, vWaterUv ).r );`):Q(X),e.fragmentShader.includes(Z)?e.fragmentShader=e.fragmentShader.replace(Z,`
  {
  ...
  ${Z}`):Q(Z)},e.customProgramCacheKey=()=>`anima-water`}function $(e,t){let n=t.material||null,r=n?.class||`matte`,i=r===`water`||r===`ice`,a={roughness:n?.roughness??(i?.08:r===`gloss`?.25:.85),metalness:n?.metalness??(i?.15:.02)};t.map?a.map=t.map:a.color=Ce(e,t.color??(i?n?.tint:16777215)),t.transparent&&(a.transparent=!0),t.opacity!==void 0&&(a.opacity=t.opacity),t.side!==void 0&&(a.side=t.side),t.depthWrite!==void 0&&(a.depthWrite=t.depthWrite);let o=new e.MeshStandardMaterial(a);return r===`glow`&&(o.emissive=Ce(e,n?.tint??16777215),o.emissiveIntensity=n?.glow??1,t.map&&(o.emissiveMap=t.map)),i&&(K||=xe(e),o.normalMap=K,o.normalScale=new e.Vector2(1,1),ye||=be(e),Te(o,n,t.mask||ye)),o}var Ee=e({AREA_EPS_M2:()=>AREA_EPS_M2,CLIP_MAX_POINTS:()=>64,CUTOUT_MAX_POINTS:()=>64,CUTOUT_MAX_POLYS:()=>16,FIGURE_HEIGHT_M:()=>FIGURE_HEIGHT_M,MAP_RELIEF_Z_FACTOR:()=>3,MAX_DECORATED_POINTS:()=>MAX_DECORATED_POINTS,SCATTER_CELLS_MAX:()=>j,SCATTER_CELL_M:()=>64,SCATTER_CLEAR_HEIGHT_RATIO:()=>w,SCATTER_MAX_PER_CELL:()=>A,SCATTER_MAX_PER_ENTRY:()=>d,SCATTER_TRIES_PER_POINT:()=>12,STROKE_AMPLITUDE_DEFAULT_M:()=>2,STROKE_SPACING_DEFAULT_M:()=>10,VERIFY_EPS:()=>VERIFY_EPS,WATERFALL_MIN_DROP_M:()=>1,WATERFALL_MIN_SLOPE:()=>WATERFALL_MIN_SLOPE,WATER_FLOW_FACTOR_MIN:()=>WATER_FLOW_FACTOR_MIN,WATER_FLOW_SPEED_DEFAULT_M_S:()=>ge,WATER_FLOW_SPEED_MAX_M_S:()=>2,surfaceMaterial:()=>$,updateSurfaceMaterials:()=>ve});export{f as C,v as D,p as E,b as O,O as S,ee as T,re as _,he as a,T as b,A as c,y as d,E as f,P as g,M as h,ce as i,l as k,d as l,m,$ as n,me as o,D as p,ve as r,j as s,Ee as t,C as u,N as v,g as w,k as x,ne as y};
//# sourceMappingURL=src-B0rk6Q_K.js.map